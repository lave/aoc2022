...
(define (find valves time-left current visited open-valves open-sum released logg)
  ...
  (if (= 0 time-left)
    (begin
      ;(if (= released 1857) (println (list "DONE" released (reverse logg))) #f)
      (if (member "MA" logg)
        (list 0 (reverse logg))
    ...
    (let* ([cur-weight (* open-sum (- time-left 1))]
           ...
           [rs (map (lambda (vv)
             (let* ([v (car vv)] [w (cadr vv)])
               (if (or (>= w time-left) (member v visited) (equal? "MA" v) (equal? "II" v))
                 (list (+ released_ cur-weight) (cons "idle" logg))
...
(define (find2 valves paths time-left current1 current2 action1 action2 visited visited1 visited2 open-valves open-sum
               released logg preset-actions1 preset-actions2)
  ;(displayln (reverse logg))
  ...
  (if (or (= 0 time-left) (= (hash-count valves) (set-count open-valves)))
    ...
    (let* (
           ...
           [actions1
             (if (< 0 (cdar action1))
               ...
               (if (empty? preset-actions1)
                 ...
                 (list (car preset-actions1))
               )
             )]
           ;[actions1_ (if (empty? actions1) (list (list (cons 'id time-left) 0 #f)) actions1)]

           [preset-actions1_ (if (and (not (empty? preset-actions1)) (equal? (car preset-actions1) (car actions1)))
                               (cdr preset-actions1) preset-actions1)]
           ...
           [actions2
             (if (< 0 (cdar action2))
               ...
               (if (empty? preset-actions2)
                 ...
                 (list (car preset-actions2))
               )
             )]
           ;[actions2_ (if (empty? actions2) (list (list (cons 'id time-left) 0 #f)) actions2)]

           [preset-actions2_ (if (and (not (empty? preset-actions2)) (equal? (car preset-actions2) (car actions2)))
                               (cdr preset-actions2) preset-actions2)]
           ...
           [results (map (lambda(actions2) (do-actions valves paths time-left current1 current2 visited visited1 visited2
                                                       open-valves open-sum released logg preset-actions1_ preset-actions2_ actions2)) actions_)]
           )
        (argmax car results)
    )
  )
...
(define (merge as1 as2)
  ;(print (list "MERGE1: " as1))
  ;(print (list "MERGE2: " as2))
...
(define (do-actions valves paths time-left current1 current2 visited visited1 visited2 open-valves open-sum released logg preset-actions1 preset-actions2 actions)
  ;(println actions)
  ...
  (find2 valves paths time-left_ current1_ current2_ atm1_ atm2_ visited__ visited1_ visited2_ open-valves__ open-sum__
         released_ logg_ preset-actions1 preset-actions2)
)
...
;(define res (find valves_ 26 "AA" '("AA") '() 0 0 '()))
;(displayln res)

(define preset-actions1 (list
  (list (cons "HR" 2) 0 "HR")
  (list (cons 'op  1) 0 "HR")
  (list (cons "DW" 2) 0 "DW")
  (list (cons 'op  1) 0 "DW")
...
(define res2 (find2 valves_ paths 26 "AA" "AA" init init (set "AA") (set "AA") (set "AA") open-valves 0 0 '()
                    ;'() '()
                    preset-actions1 preset-actions2
                    ;preset-actions1a preset-actions2a
))
(displayln res2)
